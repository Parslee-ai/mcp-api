...
AnyAPI is a dynamic MCP (Model Context Protocol) server that exposes REST APIs as MCP tools. It parses OpenAPI specifications and allows AI agents to call any registered API through a unified interface.
...
az acr build --registry <registry> --resource-group <rg> --image anyapi-web:v1 --file Dockerfile .

# Update container app
az containerapp update --name anyapi-web --resource-group <rg> --image <registry>.azurecr.io/anyapi-web:v1
```
...
- **AnyAPI.Core** - Domain models, OpenAPI parsing, storage, auth handlers, HTTP client
- **AnyAPI.Web** - Blazor Server UI for API management (register, configure, enable/disable)
- **AnyAPI.Mcp** - MCP server that exposes registered APIs as tools
...
- `Cosmos:DatabaseName` (default: "AnyApiDb")
